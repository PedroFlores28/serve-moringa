...
function buildMonthScope(referenceDate = new Date(), extraPeriodKeys = [], openSince = null) {
  const ref = referenceDate || new Date();
  const keys = new Set([calendarMonthKey(ref)]);
  for (const k of extraPeriodKeys || []) {
    if (k) keys.add(String(k));
  }
  const since = openSince ? new Date(openSince) : null;
  return {
    start: startOfCalendarMonth(ref),
    end: endOfCalendarMonth(ref),
    periodKeys: [...keys],
    openSince: since && !isNaN(since.getTime()) ? since : null,
  };
}
...
function computeMonthlyActivity(
  ...
  const personalRecomprasCount = sumRecordProductsInMonth(personalActivations, referenceDate);
  const personalAffiliationsCount = sumRecordProductsInMonth(personalAffiliations, referenceDate);

  const personalProductCount = personalRecomprasCount + personalAffiliationsCount;
  ...
  return {
    ...
    personalRecomprasCount,
    personalAffiliationsCount,
    groupProductCount,
    monthlyActive,
    affiliatedThisMonth,
...
function indexRecordsByUserId(records) {
  const map = new Map();
  for (const record of records || []) {
    if (!isApprovedRecord(record)) continue;
    const id = recordUserId(record);
    if (id == null) continue;
    const key = String(id);
...
/** Unidades de una compra. En recompra se usa `total` del checkout. */
function countPersonalUnits(record) {
  if (!record) return 0;
  if (!record.plan) {
    const header = Number(record.total);
    if (Number.isFinite(header) && header > 0 && header < 200) return header;
  }
  return countProductsInRecord(record);
}

/**
 * Parte cada compra personal en periodo vigente o acumulado anterior.
 * No hay solape: una compra entra en un lado o en el otro.
 */
function splitPersonalPeriodProducts(
  userId,
  activations,
  affiliations,
  referenceDate,
  ...
  const scope = buildMonthScope(referenceDate, extraPeriodKeys, openSince);
  const split = (records, addPrice) => {
    let current = 0;
    let previous = 0;
    let currentBs = 0;
    for (const record of records || []) {
      if (!isApprovedRecord(record)) continue;
      if (!sameUserId(recordUserId(record), userId)) continue;
      const n = countPersonalUnits(record);
      if (recordInCurrentPeriod(record, scope)) {
        current += n;
        if (addPrice) currentBs += Number(record.price) || 0;
      } else {
        previous += n;
      }
    }
    return { current, previous, currentBs };
  };
  const rec = split(activations, true);
  const aff = split(affiliations, false);
  return {
    currentRecompras: rec.current,
    currentRecomprasBs: rec.currentBs,
    currentAffiliations: aff.current,
    histRecompras: rec.previous,
    histAffiliations: aff.previous,
  };
}
...
module.exports = {
  ...
  indexRecordsByUserId,
  splitPersonalPeriodProducts,
};
