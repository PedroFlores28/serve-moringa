...
async function loadAllApprovedActivationsAndAffiliations() {
  const [activations, affiliations] = await Promise.all([
    Activation.find({ status: "approved" }),
...
function enrichPreviewTreeForClassMoringa(
  ...
  referenceDate,
  lifetimeActivations,
  lifetimeAffiliations,
  extraPeriodKeys = [],
  openSince = null
) {
  const userById = new Map((usersList || []).map((u) => [u.id, u]));
  const treeById = new Map((treeList || []).map((n) => [n.id, n]));
  const histActs = lifetimeActivations || activations;
  const histAffs = lifetimeAffiliations || affiliations;
  const activationsByUser = indexRecordsByUserId(histActs);
  const affiliationsByUser = indexRecordsByUserId(histAffs);
  ...
  return (previewTree || []).map((node) => {
    ...
    let personalRecompras = 0;
    let personalAffiliations = 0;
    let personalRecomprasBs = 0;
    let histAffiliations = 0;
    let histRecompras = 0;
    let teamProducts = 0;
    if (userDoc) {
      ...
      const split = splitPersonalPeriodProducts(
        userDoc.id,
        activationsByUser.get(String(userDoc.id)) || [],
        affiliationsByUser.get(String(userDoc.id)) || [],
        referenceDate,
        extraPeriodKeys,
        openSince
      );
      personalRecompras = split.currentRecompras || 0;
      personalAffiliations = split.currentAffiliations || 0;
      personalRecomprasBs = split.currentRecomprasBs || 0;
      histAffiliations = split.histAffiliations || 0;
      histRecompras = split.histRecompras || 0;
    }
    ...
    return {
      ...
      personal_recompras: personalRecompras,
      personal_affiliations: personalAffiliations,
      personal_recompras_bs: personalRecomprasBs,
      hist_affiliations: histAffiliations,
      hist_recompras: histRecompras,
...
module.exports = {
  ...
  loadAllApprovedActivationsAndAffiliations,
