...
const { isClassOrEmpresarioPlan } = require("./monthlyActivity");
 ...
 * Solo válida si el paquete otorgaba estatus de actividad (Class / VIP).
 */
function hasAffiliationThisMonth(user, referenceDate) {
  if (!isClassOrEmpresarioPlan({ id: user?.plan })) return false;
 ...
 * Solo válida si es paquete Class / VIP.
 */
...
/**
 * Paga el bono por patrocinio directo al patrocinador si está activo.
 * - Si el monto configurado es 0 (o no positivo), no genera bono.
 * - Por defecto 120 Bs si no se especifica.
...
async function payDirectAffiliationBonus({
  ...
  const amountToPay =
    bonusAmount !== undefined && bonusAmount !== null
      ? Number(bonusAmount)
      : DIRECT_AFFILIATION_BONUS;

  if (amountToPay <= 0) {
    console.log(
      `[Affiliation Bonus] Bono patrocinio directo configurado en Bs 0 para afiliación ${affiliationId}. No se genera transacción de bono.`
    );
    return null;
  }
  ...
  if (!active) {
    console.log(
      `[Affiliation Bonus] Patrocinador ${sponsor.id} inactivo (${reason}), bono se acumula en saldo no disponible (virtual) por afiliación ${affiliationId}`
    );
  }
  ...
  console.log(
    `[Affiliation Bonus] Bs ${amountToPay} → patrocinador ${sponsor.id} (virtual: ${!active}, reason: ${reason}) por afiliación ${affiliationId}`
  );
