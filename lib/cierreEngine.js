...
class CierreEngine {
  ...
  processCyclesAndRank(id, monthlyActiveMap) {
    const user = this.users.get(id)
    if (!user) return { maxRank: "none", qualifyingRank: "none", completedCycles: 0, overflow: 0, status: "inactivo", generatedBonus: 0 }
    ...
    const totalProducts = personalProducts + groupProducts + overflow

    const targetProducts = targetCfg.thresholdProducts
    
    if (!isActive) {
      completedCycles = Math.min(completedCycles, 1)
      return {
...
async function runCierre(opts = {}) {
  ...
  for (const u of users) {
    ...
    // Inject products into engine's user representation for cycle evaluation
  ...
  for (const u of users) {
    ...
    const hasActivity = totalPoints > 0 || res.total > 0 || gen.total > 0 || sav > 0 || (cycleRes && cycleRes.generatedBonus > 0) || (cycleRes && cycleRes.status !== "inactivo" && cycleRes.status !== "fallo_activo");
    if (hasActivity) {
      previewNodes.push({
        id: u.id,
        dni: u.dni || "",
