...
/** Misma regla que el query del periodo: period_key manda; sin key, fecha del mes. */
function recordInCurrentPeriod(record, scope) {
  if (!record || !scope) return false;
  const key =
    record.period_key != null && record.period_key !== ""
      ? String(record.period_key)
      : "";
  if (key) {
    return !!(scope.periodKeys && scope.periodKeys.includes(key));
  }
  const when = record.approved_at || record.date;
  if (!when) return false;
  const d = when instanceof Date ? when : new Date(when);
  if (isNaN(d.getTime())) return false;
  if (scope.openSince) return d >= scope.openSince;
