  ...
  try {
    ...
    if(req.method == 'GET') {
      ...
      offices = offices.map(office => {
        ...
        office.products = (Array.isArray(office.products) ? office.products : []).map(p => {
    ...
    if(req.method == 'POST') {
      ...
      if(products) {
        ...
        if (!office) {
          return res.status(404).json({ error: true, message: "Oficina no encontrada" })
        }

        products.forEach((p, i) => {
          // console.log({ i , p })
          if (!office.products[i]) return
    ...
    console.error("Error en /api/admin/offices:", error)
    return res.status(500).json({ error: true, message: "Error interno del servidor" })
  }
}
